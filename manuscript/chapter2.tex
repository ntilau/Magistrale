...
\begin{figure}[ht!]
\centering
\includegraphics[width=12cm,keepaspectratio]{SVDerrorExp_17}
\caption{Error plot relative to the number of vectors $\mat{\psi}_{(\theta_x\ensuremath{^\circ},0\ensuremath{^\circ})}$ up to 17 (left) and singular values related to the scan angle space built with 17 vectors (right) for an array of 17 by 1 point sources equally spaced of $\nicefrac{\lambda}{2}$.}
...
\begin{figure}[htbp!]
\centering
\includegraphics[width=12cm,keepaspectratio]{SVDerrorLin_17}
\caption{Error on the tested angle of 23.3251\ensuremath{^\circ} and singular values for 33 vectors selected linearly with $\theta_x = 0\ensuremath{^\circ} \to 90\ensuremath{^\circ}$ (17 point sources linear array, spacing of $\nicefrac{\lambda}{2}$).}
\label{fig:SVDerrorLin_17}
\end{figure}
\begin{figure}[hbtp!]
\centering
\includegraphics[width=12cm,keepaspectratio]{SVDerrorInvLin_17}
\caption{Error on the tested angle of 23.3251\ensuremath{^\circ} and singular values for 33 vectors selected linearly with $\theta_x = 90\ensuremath{^\circ} \to 0\ensuremath{^\circ}$ (17 point sources linear array, spacing of $\nicefrac{\lambda}{2}$).}
\label{fig:SVDerrorInvLin_17}
\end{figure}
\par To see whether the choice of the spanning angles is important while building $\mat{A}_\theta$, let us try two other sequences: the linear sequences from $\theta_x = 0\ensuremath{^\circ} \to 90\ensuremath{^\circ}$ and \itt{vice versa}, with a fixed $\Delta \theta_x$. We have chosen up to 33 vectors ($\Delta \theta_x = 2.8125\ensuremath{^\circ}$) and the resulting errors are show in figures \ref{fig:SVDerrorLin_17} and \ref{fig:SVDerrorInvLin_17}. In the first case, the tested angle of 23.3251\ensuremath{^\circ} become included in the scan angle space interval $[0\ensuremath{^\circ}, 25.3125\ensuremath{^\circ}]$ with the $10^\mrm{th}$ vector ($\vect{\psi}_{(25.3125\ensuremath{^\circ},0\ensuremath{^\circ})}$), but the error begins its decreasing slope 5 vectors before, and the error becomes numerically null at the $17^\mrm{th}$ vector (size of the array). In the second case, we need to wait until the $25^\mrm{th}$ vector ($\vect{\psi}_{(22.5\ensuremath{^\circ},0\ensuremath{^\circ})}$) to see the error dropping below the numerical noise floor ($< 10^{-13}$). The final 33 vectors, being the same in the two cases, are such that, obviously, we achieve the same singular values.
\begin{figure}[!hbtp]
\begin{minipage}[b]{0.5\linewidth} % A minipage that covers half the page
\centering
\includegraphics[width=7cm,keepaspectratio]{SVDerrorLin_17_10vects}
\caption{Singular values of the space spanned by 10 vectors in a linear selection of angles from $0\ensuremath{^\circ} \to 90\ensuremath{^\circ}$.}
\label{fig:SVDerrorLin_17_10vects}
\end{minipage}
\hspace{0.1cm} % To get a little bit of space between the figures
\begin{minipage}[b]{0.5\linewidth}
\centering
\includegraphics[width=7cm,keepaspectratio]{SVDerrorInvLin_17_25vects}
\caption{Singular values of the space spanned by 25 vectors in a linear selection of angles from $90\ensuremath{^\circ} \to 0\ensuremath{^\circ}$.}
...
\begin{minipage}[b]{0.5\linewidth}
\centering
\includegraphics[width=7cm,keepaspectratio]{SVDerrorExp_19}
\caption{Error plot for an array of 19 point sources, spacing of $\nicefrac{\lambda}{2}$.}
\label{fig:SVDerrorExp_19}
\end{minipage}
\end{figure} The number of vectors that can be used as a basis for the fields produced by an array of 18 point sources, for the accuracy chosen of $10^{-3}$, can be of 14. This is true for any test angle in the range $[0\ensuremath{^\circ},90\ensuremath{^\circ}]$. In effect, several angles have been tested, remaining in the middle of two spanning angles for the basis. The pattern resulting from a linear array of 21 point sources, steered at $\theta_x = 2.8125\ensuremath{^\circ}$ and projected onto a basis of 17 vectors built by an exponential scan angle selection is shown in figure \ref{fig:pattern_21_2,8125_17}.
\begin{figure}[htp!]
\centering
\includegraphics[width=12cm,keepaspectratio]{pattern_21_2_8125_17}
\caption{Pattern of the fields produced by a linear array of 21 point sources steered at 2.8125\ensuremath{^\circ} and tested by a 17 vectors basis.}
\label{fig:pattern_21_2,8125_17}
\end{figure} As the N2F error, being of 0.01984 (n.f. error = 0.01724), has superseded our accuracy threshold, we can see some misfits on the lateral lobes. However, for practical applications, this can be acceptable. Now testing the pattern steered at $5 \cdot 2.8125\ensuremath{^\circ} = 14.0625\ensuremath{^\circ}$ (figure \ref{fig:pattern_21_14,0625_17}), we obtain a N2F error of $1.55 \ 10^{-3}$ (n.f. error = $1.342 \ 10^{-3}$). Then for $11 \cdot 2.8125\ensuremath{^\circ} = 30.9375\ensuremath{^\circ}$ (figure \ref{fig:pattern_21_30,9375_17}) and for $21 \cdot 2.8125\ensuremath{^\circ} = 59.0625\ensuremath{^\circ}$ (figure \ref{fig:pattern_21_59,0625_17}) we have, respectively, the errors $5.48 \ 10^{-5}$ (n.f. error = $5.042 \ 10^{-5}$) and $1.92 \ 10^{-7}$ (n.f. error = $2.82 \ 10^{-7}$).
\begin{figure}[htp!]
\centering
\includegraphics[width=12cm,keepaspectratio]{pattern_21_14_0625_17}
\caption{Pattern of the fields produced by a linear array of 21 point sources steered at 14.0625\ensuremath{^\circ} and tested by a 17 vectors basis.}
\label{fig:pattern_21_14,0625_17}
\end{figure}

\begin{figure}[htp!]
\centering
\includegraphics[width=12cm,keepaspectratio]{pattern_21_30_9375_17}
\caption{Pattern of the fields produced by a linear array of 21 point sources steered at 30.9375\ensuremath{^\circ} and tested by a 17 vectors basis.}
\label{fig:pattern_21_30,9375_17}
\end{figure}
\begin{figure}[htp!]
\centering
\includegraphics[width=12cm,keepaspectratio]{pattern_21_59_0625_17}
\caption{Pattern of the fields produced by a linear array of 21 point sources steered at 59.0625\ensuremath{^\circ} and tested by a 17 vectors basis.}
...
\begin{figure}[hbtp!]
\centering
\includegraphics[width=12cm,keepaspectratio]{errorExpAS}
...
\begin{figure}[hbtp!]
\centering
\includegraphics[width=12cm,keepaspectratio]{errorAvg}
...
\begin{minipage}[b]{0.5\linewidth} % A minipage that covers half the page
\centering
\includegraphics[width=7cm,keepaspectratio]{nfSolid30}
\caption{Near field solid $|\psi|$ generated by a linear array of 9 point sources on the $x$ direction steered at 30\ensuremath{^\circ}.}
\label{fig:nfSolid30}
\end{minipage}
\hspace{0.1cm} % To get a little bit of space between the figures
\begin{minipage}[b]{0.5\linewidth}
\centering
\includegraphics[width=7cm,keepaspectratio]{nfSolid30rot}
\caption{Near field solid $|\psi|$ generated by a linear array of 9 point sources on the $x$ direction steered at 30\ensuremath{^\circ} rotated 30\ensuremath{^\circ} backward.}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=12cm,keepaspectratio]{SVDerrorExpRotated}
...
\begin{figure}[hbtp!]
\centering
\includegraphics[width=12cm,keepaspectratio]{SVDerrorExpRotatedMax}
...
\end{eqnarray} where $\mathcal{L}$ is a linear\footnote{The surfaces enclosing the sources can be more than one, an the far field detector results in the sum of the application of the operator on each surface.} functional parameterized in the look angle direction $\hat{\mbfit{r}} \in \mathcal{D} \subset \mathbb{R}^3$. In a first approach, we will restrain the look angle domain $\hat{\mbfit{r}} \in \mathcal{D}_{\phi = 0\ensuremath{^\circ}} \subset \mathcal{D}$, seeking for an approximation of $\mathcal{L}(\hat{\mbfit{r}})$ along the pattern cut plane XZ ($\phi = 0\ensuremath{^\circ}$).

\par The main idea of the interpolation techniques presented below is to construct basis functions with a large information content in order to reduce the number of basis coefficients needed to reach a certain level of accuracy in the outputs of interest. %In a similar way as does the POD (section \ref{sec:POD}), we seek for the optimal basis to approximate the functional, in order to minimize the number of coefficients required.

\subsection{Polynomial interpolation of the N2F operator} \label{sec:PolynomN2F}

\par Given $\hat{\mbfit{r}} \in \mathcal{D}_{\phi = 0\ensuremath{^\circ}}$, we first introduce parameter samples $\vect{S}_P \equiv \left \{ \hat{\mbfit{r}}_1, \ldots, \hat{\mbfit{r}}_P \right \},$ that is the chosen look angles for pattern plotting, and associated \itt{operator snapshots}  $\mat{L}_P = \colspan{ \vect{\lambdaup}_p \equiv \mathcal{L}(\hat{\mbfit{r}}_p) , 1 \leq p \leq P}.$ The column vector of far field detectors $\vect{\psi}_r^*$ is given by the matrix vector product
\begin{eqnarray}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=10cm,keepaspectratio]{operatorSVD}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=8cm,keepaspectratio]{1st}
\caption{\itt{Roof-top} interpolating functions for the first 3 nodes.}
\label{fig:1st}
\end{figure}
For each node $q$, the roof-top function can be defined as, being $\hat{\mbfit{r}} = \hat{\mbfit{r}} (\theta, \phi=0\ensuremath{^\circ}) =  \hat{\mbfit{r}} (\theta) $
\begin{eqnarray}
\Theta_q (\hat{\mbfit{r}}) & = & \left \{  \begin{array}{c l}
\frac{\theta \ - \ \theta_{q-1}}{\theta_q \ - \ \theta_{q-1}}, & \qquad \hat{\mbfit{r}} (\theta_{q-1}) \in \mathcal{D}_{\phi = 0\ensuremath{^\circ}}, \hat{\mbfit{r}} (\theta) \in \mathcal{Q}_q,\\
\frac{\theta_{q+1} \ - \ \theta}{\theta_{q+1} \ - \ \theta_{q}}, & \qquad \hat{\mbfit{r}} (\theta_{q+1}) \in \mathcal{D}_{\phi = 0\ensuremath{^\circ}}, \hat{\mbfit{r}} (\theta) \in \mathcal{Q}_q,\\
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=8cm,keepaspectratio]{2nd}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=12cm,keepaspectratio]{error2ndShape}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=8cm,keepaspectratio]{3rd}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=12cm,keepaspectratio]{error3rdShape}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=12cm,keepaspectratio]{errorShape3_10}
...
\end{eqnarray} where we have assumed $\hat{\mbfit{r}} \in \mathcal{D}_{\phi = 0\ensuremath{^\circ}}$. Using \itt{Euler's formulas} with $\mrm{e}^{i m \theta} = \cos(m \theta)  + i \sin(m \theta) $, $i$ being the imaginary unit, \eqref{eq:ApproxSumTrig} can be rewritten as \cite{BrighamFFT}
\begin{eqnarray}
...
\end{equation}
The assumption of the periodicity of $\mat{x}(n)$, in our case, is valid as long as our operator is built by scanning $\theta$ all over 360\ensuremath{^\circ}.
...
\par It is now possible to apply the DFT-truncation to a selected N2F problem. We chose, in order to compare the results to those obtained by the FEM basis based approximation, a linear array of 5 point sources within a sphere of radius of $\nicefrac{3 \ \lambda}{2}$ and sampling resolution of $\nicefrac{\lambda}{10}$. The number of look angles in order to build the operator matrix of snapshots $\mat{x}_N$ is of $N = 300$ in the range [0\ensuremath{^\circ}, 360\ensuremath{^\circ}). Figure \ref{fig:DFTspectrum} shows the spectral coefficients $\mat{X}$ computed by an FFT on the rows (look angle dependent) of $\mat{x}_N$.

\begin{figure}[!hbtp]
\centering
\includegraphics[width=12cm,keepaspectratio]{DFTspectrum}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=12cm,keepaspectratio]{errorDFT_5}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=12cm,keepaspectratio]{errorDFT_10}
